"""Guarded-paging ablation harness; historical runs are bound to their source receipts."""
...
from mercury.lexical.diagnostics import constraint_receipts, stage_receipt
from mercury.lexical.paging import explicit_override, select_page, semantic_signature
...
class PagingAgent:
    """Ablation adapter using the production selector on an explicitly unpaged base."""
    def __init__(self, catalog: Path, *, enabled: bool, inner=None):
        self.base = inner or PresentationAgent(catalog)
        if self.base.inner.config.guarded_paging:
            raise ValueError("Paging ablations require an unpaged base agent")
...
def run(catalog: Path, dataset: Path, output: Path, enabled: bool):
    ...
    registration = {'sources': before, 'config': asdict(replace(DEFAULT_AGENT_CONFIG, guarded_paging=False)),
                    'paging_enabled': enabled,
