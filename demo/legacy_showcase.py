"""Historical neural showcase. For the public lexical pipeline use demo.submission."""
...
def _render_html(report: dict) -> str:
    ...
    for turn in report["turns"]:
        ...
        override_reason = ", ".join(override.get("reasons", [])) or "No override detected"
        changed_attributes = ", ".join(override.get("changed_attributes", [])) or "None"
        turn_sections.append(
            ...
            f'<dt>Intent revision</dt><dd>{html.escape(override_reason)}</dd>'
            f'<dt>Changed attributes</dt><dd>{html.escape(changed_attributes)}</dd>'
