...
_COMMITTED = re.compile(
    ...
    r"non[- ]negotiable|deal[- ]?breaker)\b", re.I,
)
_DIRECT_REQUEST = re.compile(
    r"\b(?:i\s+(?:need|want)|i(?:'m| am)\s+(?:looking for|after)|"
    r"find me|show me|please find)\b", re.I,
)
_EXPLORATORY = re.compile(
    r"\b(?:ideas?|explor(?:e|ing)|browse|browsing|inspiration|suggestions?|"
    r"not sure|figuring out|open to|maybe|might|possibly|considering)\b", re.I,
)
_SCENARIO_BROWSING = re.compile(r"\b(?:something for|gift(?:s|ing)?|occasion)\b", re.I)
_NONCOMMITMENT = re.compile(r"\bwithout\s+(?:choosing|committing|settling|deciding)\b", re.I)
...
_MIXED = re.compile(
    ...
    r"(?:heavy|substantial).*(?:weightless|no heft))\b", re.I,
)
_RELAXATION = re.compile(
    r"\b(?:forget|drop|remove|release|does(?:n't| not) matter|no preference|"
    r"anything is fine|whatever works|any (?:color|material|style|brand|size))\b", re.I,
)
_CORRECTION = re.compile(
    r"\b(?:not that|i meant|correction|scratch that|don't want|do not want)\b", re.I,
)
@dataclass(frozen=True, slots=True)
class IntentWeights:
    """Interpretable intent-rule weights fitted without evaluator targets."""
...
def decide_intent(state: SessionState, message: str, buying_threshold: float = 0.50,
    ...
    """Classify a turn from live state and shopper language, never target labels."""
    ...
    hard_count = sum(preference.hard for preference in active)
    ...
    distinct_slots = len({
        preference.attribute for preference in positives if preference.attribute != "other"
    })
    ...
    direct_request = (
        direct_request_phrase and not browsing_language
    ...
    buying_score = (
        weights.object * has_object + weights.slots * min(distinct_slots, 3)
        + weights.hard * bool(hard_count)
    )
    browsing_score = (
        weights.browsing_language * browsing_language
    ...
    if _RELAXATION.search(message) or removed_without_replacement:
        ...
        event = "correction"
          ...
          or state.last_override.detected):
        event = "override"
    else:
        event = "continue"
    if event != "continue":
